...
async def harvest_pool_data():
    ...
    # Create a custom httpx client to handle the timeouts
    custom_timeout = httpx.Timeout(60.0, connect=60.0)
    
    async with httpx.AsyncClient(timeout=custom_timeout, http2=True) as httpx_client:
        # Pass the custom client into AqualinkClient
        async with AqualinkClient(USERNAME, PASSWORD, client=httpx_client) as client:
            try:
